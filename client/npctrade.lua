...
local function NearPed(npcmodel, npccoords, id, name)
    ...
    if Config.EnableTarget then
        exports.ox_target:addLocalEntity(spawnedPed, {
            {
                name = 'npc_collectablecards_trade',
                icon = 'fa-solid fa-eye',
                label = name,
                targeticon = 'fa-solid fa-box',
                onSelect = function()
                    TriggerEvent('tilp-hdrp-collectablecards:client:openmission', id)
                end,
                distance = 3.0
            }
        })
...
CreateThread(function()
    while true do
        Wait(500)
        for k,v in pairs(Config.CardMission) do
...
CreateThread(function()
    while true do
        Wait(500)
        for _,v in pairs(Config.CardMission) do
            if not Config.EnableTarget then
                exports['rsg-core']:createPrompt(v.id, v.coords, RSGCore.Shared.Keybinds[Config.Missions.Key], v.name, {
                    type = 'client',
                    event = 'tilp-hdrp-collectablecards:client:openmission',
                    args = { v.id },
                })
            end

            if Config.Blip.showblipCollect == true then
                local blipCardshop = BlipAddForCoords(1664425300, v.coords)
                SetBlipSprite(blipCardshop, joaat(Config.Blip.CollectSprite), true)
                SetBlipScale(blipCardshop, Config.Blip.CollectScale)
                SetBlipName(blipCardshop, Config.Blip.CollectName)
            end
        end
    end
end)
